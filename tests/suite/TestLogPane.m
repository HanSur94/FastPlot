classdef TestLogPane < matlab.unittest.TestCase
%TESTLOGPANE Class-based tests for LogPane (Phase 1027).
%   Covers attach/detach lifecycle, buffer preservation across re-attach,
%   buffer cap, theme propagation, clearLiveLog, and DetachRequested event firing.
%
%   MATLAB-only — Octave skipped (uifigure unavailable).
%
%   See also LogPane, TestFastSenseCompanion, run_all_tests.
    ...
    methods (TestMethodSetup)
        function skipOnOctave(testCase)
            testCase.assumeFalse( ...
                ...
                'TestLogPane: skipped on Octave (uifigure not available)');
        end
    end
    ...
    methods (Test)
        ...
        function testConstructorRequiresTheme(testCase)
            %TESTCONSTRUCTORREQUIRESTHEME Missing/non-struct theme -> LogPane:invalidTheme.
            testCase.verifyError(@() LogPane(), 'LogPane:invalidTheme');
            testCase.verifyError(@() LogPane(42), 'LogPane:invalidTheme');
        end

        function testConstructDoesNotBuildUI(testCase)
            %TESTCONSTRUCTDOESNOTBUILDUI Construction must NOT attach (deferred to attach()).
            p = LogPane(CompanionTheme.get('dark'));
            testCase.addTeardown(@() delete(p));
            testCase.verifyFalse(p.IsAttached, ...
                'LogPane should be detached after construction');
        end
        ...
        function testDetachClearsHandles(testCase)
            ...
            testCase.verifyEqual(p.bufferSize('log'), 1);
        end
        ...
        function testDetachPreservesBuffers(testCase)
            ...
            testCase.verifyEqual(p.bufferSize('log'), 5, ...
        ...
        function testFiveHundredRowCapLog(testCase)
            %TESTFIVEHUNDREDROWCAPLOG LogBuffer_ caps at 500 rows.
            [p, ~] = testCase.makePane_('dark');
            for i = 1:600
                p.addLogEntry('info', sprintf('line %d', i));
            end
            testCase.verifyEqual(p.bufferSize('log'), 500, ...
                'log buffer should cap at 500');
        end

        function testFiveHundredRowCapLive(testCase)
            %TESTFIVEHUNDREDROWCAPLIVE LiveLogBuffer_ caps at 500 rows.
            [p, ~] = testCase.makePane_('dark');
            for i = 1:600
                p.addLiveLogEntry('tag.x', 1, double(i));
            end
            testCase.verifyEqual(p.bufferSize('live'), 500, ...
        ...
        function testClearLiveLog(testCase)
            ...
            testCase.verifyEqual(p.bufferSize('live'), 3);
            p.clearLiveLog();
            testCase.verifyEqual(p.bufferSize('live'), 0);
        end
        ...
        function testLogBufferOrdering(testCase)
            ...
            testCase.verifyEqual(p.bufferSize('log'), 2);
    ...
    methods (Access = private)
        ...
        %MAKEPANE_ Helper: build a hidden uifigure and an attached LogPane.
