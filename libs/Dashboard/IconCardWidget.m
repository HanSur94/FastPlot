...
%   See also DashboardWidget, StatusWidget, NumberWidget.
    ...
    methods
        ...
        %RENDER Create icon, value text, and label inside parentPanel.
            ...
            % Adaptive font size from content area (panel minus the 28-px
            % button bar at the top), so text isn't clipped under the bar.
            [~, ~, pHContent, contentTop] = obj.panelMetrics_(parentPanel);
            fontSz = max(7, min(14, round(pHContent * 0.34)));

            % All sub-controls fit within [0, contentTop] vertically.
            yMid = contentTop / 2;
            iconY = max(0.05, yMid - 0.35 * contentTop);
            iconH = max(0.20, 0.70 * contentTop);
            valueY = max(0.05, yMid);
            valueH = max(0.20, contentTop - valueY - 0.02);
            labelY = 0.02;
            ...
            obj.hIconAx = axes('Parent', parentPanel, ...
                'Units', 'normalized', ...
                'Position', [0.02 iconY 0.16 iconH], ...
                'Visible', 'off', ...
            ...
            obj.hValueText = uicontrol('Parent', parentPanel, ...
                ...
                'String', '--', ...
                'Units', 'normalized', ...
                'Position', [0.20 valueY 0.75 valueH], ...
                'FontName', fontName, ...
                'FontSize', fontSz + 2, ...
                'FontWeight', 'bold', ...
            ...
            obj.hLabelText = uicontrol('Parent', parentPanel, ...
                ...
                'String', '', ...
                'Units', 'normalized', ...
                'Position', [0.20 labelY 0.75 labelH], ...
                'FontName', fontName, ...
