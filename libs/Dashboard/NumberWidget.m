...
%     - A struct with fields: value, unit, trend ('up'/'down'/'flat')
    ...
    methods
        ...
        function render(obj, parentPanel)
            ...
            % Adaptive font sizes based on the content area (panel height
            % minus the 28-px button bar at the top), so values aren't
            % clipped under the bar on short panels.
            [~, ~, pHContent, contentTop] = obj.panelMetrics_(parentPanel);

            valueFontSz = max(8, min(28, round(pHContent * 0.55)));
            titleFontSz = max(7, min(14, round(pHContent * 0.27)));
            trendFontSz = max(6, min(16, round(pHContent * 0.30)));

            % All uicontrols span [yBot, contentTop] vertically, leaving
            % the top strip clear of the button bar.
            yBot = 0.02;
            yH   = max(0.05, contentTop - yBot);
