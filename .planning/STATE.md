...
last_activity: 2026-05-13 -- Quick task 260513-q7w: Debounced post-resize refresh stops widgets going white during drag-resize, with proper switchPage interaction — TWO parallel timers (300 ms cheap two-pass refresh + 1.2 s unconditional rerenderWidgets backstop), switchPage cancels both via cancelResizeTimers_() helper so stale backstops don't fire on the new tab, IsRerendering_ flag prevents rerender-cascade scheduling.
progress:
  total_phases: 6
  completed_phases: 2
  total_plans: 13
...
| 260513-q7w | Debounced post-resize refresh stops widgets going white during drag-resize — TWO parallel timers on every figure resize event: (1) 300 ms `ResizeDebounceTimer` runs a cheap two-pass sweep — Pass 1 calls `update()`/`refresh()` on every active-page widget, Pass 2 detects any FastSenseWidget whose line lost XData and escalates to per-widget refresh() then rerenderWidgets() if still white; (2) 1.2 s `ResizeFinalRedrawTimer` unconditionally calls `rerenderWidgets()` as a bulletproof backstop. Both timers restart on every resize event so neither fires during continuous drag. Critical interaction fixes: `switchPage` now cancels both timers via new `cancelResizeTimers_()` helper (a stale backstop firing on a different active page after the user clicked a tab caused regression with white widgets on the new page); new `IsRerendering_` flag (set via onCleanup) makes `onResize` early-return during a rerender cascade (panel teardown SizeChangedFcn events were scheduling new timers, recursion risk); re-entrancy guard now aborts instead of self-rescheduling. All teardown wired in delete(). test_dashboard_range_selector_integration 2/2, test_dashboard_time_sync_all_pages 5/5; live demo reproduction script confirms zero white widgets after resize→tab-switch sequence | 2026-05-13 | 577bf95, 99c8808, 4eda604, bc305dc | — | [260513-q7w-during-dashboard-figure-resize-fastsense](./quick/260513-q7w-during-dashboard-figure-resize-fastsense/) |
