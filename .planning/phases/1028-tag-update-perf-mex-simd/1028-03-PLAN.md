...
Add to the SensorThreshold MEX block (added in plan 02 task 1) — append to `sensorMexFiles`:
```matlab
sensorMexFiles = {
...
If plan 02 is not yet merged, this plan adds the SensorThreshold block from scratch (same shape as in plan 02 task 1).
...
**Step E — Run regression + harness:**

```matlab
install();
runtests({'tests/suite/TestMonitorTagFSMParity.m', ...
          'tests/suite/TestMonitorTagFSMProperty.m', ...
...
fprintf('Post-K2: NoIO tickMin=%.4f s\n', r.tickMin);
```
...
Add subsection `### Post-K2 (monitor_fsm_mex landed)` with the same row format as Post-K1: NoIO tickMin/median, Δ vs baseline, D-08 status, FSM share of tick (if tBreakdown wired), Stage 1 ship-criterion progress.
  ...
  <verify>
    <automated>octave --no-gui --eval "addpath(pwd); install(); assert(exist('monitor_fsm_mex','file')==3,'kernel did not build'); r1=runtests({'tests/suite/TestMonitorTagFSMParity.m','tests/suite/TestMonitorTagFSMProperty.m'}); r2=runtests({'tests/suite/TestMonitorTag.m','tests/suite/TestMonitorTagAppend.m','tests/suite/TestMonitorTagPersistence.m'}); r3=runtests('tests/suite/TestTagPerfRegression.m'); assert(all(~[r1.Failed]) && all(~[r2.Failed]) && all(~[r3.Failed]),'parity OR public OR D-08 regression');"</automated>
  </verify>
