...
Append to `sensorMexFiles` (the SensorThreshold MEX block from plans 02/03):
```matlab
...
**Step C — Run regression + harness:**
```matlab
install();
runtests({'tests/suite/TestCompositeTag.m', ...
          'tests/suite/TestCompositeMergeParity.m', ...
          'tests/suite/TestCompositeMergeInvariants.m', ...
          'tests/suite/TestAggregateMatrixParity.m', ...
          'tests/suite/TestTagPerfRegression.m'});
% Specifically the gated bench:
bench_compositetag_merge();   % MUST exit 0 — <200 ms gate intact
bench_tag_pipeline_1k('--smoke');
r = bench_tag_pipeline_1k();
fprintf('Post-K3+K4: NoIO tickMin=%.4f s\n', r.tickMin);
```
...
After this task, mark Stage 1 Final complete in VERIFICATION.md and record the Stage 2 Trigger evaluation:
```markdown
...
**Decision:** Stage 2 (plan 06 — A1+A2 listener coalescing) is `<approved | deferred>` because `<H8+H9 > 25%, ship Stage 2 | H8+H9 ≤ 25%, deferred to follow-up phase>`.
```

If H8+H9 ≤ 25%, plan 06 becomes a documentation-only plan stating "Stage 2 deferred — Stage 1 cleared the workload bar" (this is OK and is the conditional gate per CONTEXT.md D-05).
  </action>

  <verify>
    <automated>octave --no-gui --eval "addpath(pwd); install(); r1=runtests({'tests/suite/TestCompositeTag.m','tests/suite/TestCompositeMergeParity.m','tests/suite/TestCompositeMergeInvariants.m','tests/suite/TestAggregateMatrixParity.m'}); r2=runtests('tests/suite/TestTagPerfRegression.m'); bench_compositetag_merge(); bench_tag_pipeline_1k('--smoke'); assert(all(~[r1.Failed]) && all(~[r2.Failed]),'composite suite OR D-08 regressed');"</automated>
  </verify>
