...
<task type="auto" tdd="false">
  ...
  <read_first>
    ...
    - .planning/phases/1028-tag-update-perf-mex-simd/1028-CONTEXT.md D-03 (profile-first, ≥5× rule of thumb), D-12 (.mat I/O dominance flagging)
  </read_first>

  <action>
**Step A — Run baseline measurement.**

Invoke the harness via the matlab MCP tool to capture baseline numbers on the dev machine (a proxy until CI run lands):
```matlab
install();
r_noio = bench_tag_pipeline_1k();           % NoIO, gated (gate is currently inf)
r_io   = bench_tag_pipeline_1k('Mode', 'WithIO');  % diagnostic
fprintf('NoIO   tickMin=%.4f s  tickMedian=%.4f s\n', r_noio.tickMin, r_noio.tickMedian);
fprintf('WithIO tickMin=%.4f s  tickMedian=%.4f s\n', r_io.tickMin,   r_io.tickMedian);
```

If matlab MCP is unavailable, run via `octave --no-gui` shell command. Record both NoIO and WithIO numbers.
...
| Mode | Octave (dev machine) | MATLAB (dev machine) | CI Octave | CI MATLAB |
|------|----------------------|----------------------|-----------|-----------|
| NoIO `tickMin`    | <X> ms | <Y> ms | TBD     | TBD     |
| NoIO `tickMedian` | <X> ms | <Y> ms | TBD     | TBD     |
| WithIO `tickMin`  | <X> ms | <Y> ms | TBD     | TBD     |

Notes:
- Dev machine: <macOS arm64 / linux x86_64 / etc.>
- Octave version: <ver>
- MATLAB version: <ver>
