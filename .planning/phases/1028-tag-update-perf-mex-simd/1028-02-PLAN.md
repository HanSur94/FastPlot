...
**Step C — Run smoke + harness + full regression.**

Use the matlab MCP (or shell `octave --no-gui`):
```matlab
install();   % rebuilds MEX
% Smoke
bench_tag_pipeline_1k('--smoke');
% Regression
runtests('tests/suite/TestTagPerfRegression.m');
% Parity
runtests('tests/suite/TestDelimitedParseParity.m');
% Full bench
r1k = bench_tag_pipeline_1k();
r1k_io = bench_tag_pipeline_1k('Mode', 'WithIO');
fprintf('Post-K1: NoIO tickMin=%.4f s; WithIO tickMin=%.4f s\n', r1k.tickMin, r1k_io.tickMin);
```
